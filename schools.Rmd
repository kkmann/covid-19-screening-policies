...
## Current Government Policy (default)

The current default policy (after reopening of schools) is to rely on symptomatic
isolation of any pupil with symptoms and the entire corresponding bubble.
We assume that all individuals isolate for 14 days.
...
tbl_policies = DataFrame(
	name = ["default", "extended weekend", "regular screening", "test & release"],
	schooldays = [collect(0:4), collect(0:2), collect(0:4), collect(0:4)],
	policy = [
		SymptomaticIsolation(14, true),
		SymptomaticIsolation(14, true),
		RegularScreening(lfd; isolate_all = false, test_weekdays = [0]),
...
```{julia, results="hide"}
school = sample_school(.033, collect(0:4), DoNothing())
...
ggraph(as_tbl_graph(A), "matrix") +
	...
	labs(
		caption = "Adjacency matrix of the proposed population model;\r\nconnection strength is given in expected pairwise daily risk contacts across all shared groups."
	)
```
...
population is simulated for 21 days.
...
tbl_scenarios <- tibble(
	...
	expand_grid(
		iter = 1:100,
		name = c("default", "extended weekend", "regular screening", "test & release")
	)
...
@everywhere function evaluate_school(gamma, schooldays, policy)
	...
	for i in 1:(12*7)
		for indv in school.individuals
			if rand(Bernoulli(1/length(school.individuals)/7))
				infect!(indv)
			end
		end
    	step!(school)
...
```{r plot-simulation-results, fig.height=8}
tbl_results %>%
