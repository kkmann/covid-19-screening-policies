...
tbl_policies = DataFrame(
	...
	policy = [
		...
		SplitGroup(a_days = [0, 1], b_days = [2, 3]; pcr_turnaround = 2, isolation_duration = 8)
	]
)
...
tbl_scenarios <- tibble(
	...
	expand_grid(
		...
		pr_external_infections = c(1, 2, 3)/12/27/7 # expected 1 per week
	) %>%
	expand_grid(
		iter = 1:100,
...
```{r plot-simulation-results, fig.height=15}
...
save_plot(plt, "results", height = 18)
print(plt)
```
