...
match_gamma1 <- function(r) {
	uniroot(function(x) gamma(r, params_baseline) - gamma(x, params), interval = c(r/2, 2.5*r))$root
}
match_gamma2 <- function(r) {
	uniroot(function(x) gamma(x, params_baseline) - gamma(r, params), interval = c(r/5, r/2))$root
}
...
plt1 <- tbl_results %>%
	...
	ggplot() +
		aes(policy_name, `% infected`, color = `bubbles per class`) +
...
save_plot(plt, "fig5-three-vs-one-bubble", width = doublecolwidth, height = 2*height)
