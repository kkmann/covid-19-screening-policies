...
tbl_results <- as_tibble(julia_eval("tbl_results")) %>%
	rename(
		...
		`Pr[ymptomatic]` = frac_symptomatic,
...
```{r plot-simulation-results, fig.height=18}
plot_outcome <- function(outcome) {
	plt <- tbl_results %>%
	transmute(
		name, 
		r0, 
		`Pr[ymptomatic]` = sprintf("%.2f", `Pr[ymptomatic]`), 
		`Pr[external infection]` = sprintf("%.3f", `Pr[external infection]`), 
		iter, 
		value = !!sym(outcome)
	) %>%
	ggplot() +
		...
		facet_grid(
			`Fraction symptomatic` ~ `Pr[external infection]`, labeller = label_both
		) +
		labs(
			x = "r zero",
			y = "",
